"""
Phase B, Day 2-3 — fine-tune BreizhCrops' pretrained TempCNN.
...
one). Output of this script is a checkpoint that dl_service.py (Day 5)
will load for inference.
...
Run on Colab (recommended — free GPU) or locally with a CUDA GPU if
you have one; CPU works too, just slower (TempCNN is small: ~1D
convs over a 45-timestep, 13-band sequence, so even CPU fine-tuning
for a few epochs is plausible if GPU isn't available):

    pip install breizhcrops
    python finetune_dl_service.py --datapath ./breizhcrops_dataset --epochs 8

First run downloads real Sentinel-2+RPG data from BreizhCrops' S3
bucket (a few GB depending on level/region) — this sandbox can't
reach that bucket, so this hasn't been execution-tested end-to-end
here; the loader calls and training loop below are copied directly
from BreizhCrops' own examples/train.py pattern and its pretrained()
loader, both read directly from the cloned repo.
"""
import argparse
import os
